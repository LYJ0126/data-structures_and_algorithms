...
private:
	...
	void print(RBNode* node, int key, int direction);//打印红黑树
public:
	RBT();//构造函数
	~RBT();//析构函数
	void inorder();//中序遍历
	...
	int minimum();//查找最小值
	int maximum();//查找最大值
	bool insert(int key);//插入。因为我们这里的红黑树不允许有相同键值的节点的，所以插入时如果有相同键值的节点，我们就不插入,返回false
...
protected:
	...
	void leftRotate(RBNode* x);//左旋
	void rightRotate(RBNode* x);//右旋
	void insertFixup(RBNode* s);//插入修正
	void remove(RBNode* z);//删除
	void removeFixup(RBNode* x);//删除修正
	void transplant(RBNode* u, RBNode* v);//用v替换u
};